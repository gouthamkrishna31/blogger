...
func TestInMemoryPostStrategy_GetPost(t *testing.T) {
    // Create a mock controller
    ctrl := gomock.NewController(t)
    defer ctrl.Finish()

    // Create a mock PostStrategy
    mockStrategy := NewMockPostStrategy(ctrl)
    ...
    // Set up expectations
    mockStrategy.EXPECT().GetPost(gomock.Any(), req).Return(expectedResp, nil)

    // Call the GetPost method of the strategy
    resp, err := mockStrategy.GetPost(context.Background(), req)
    if err != nil {
...
func TestInMemoryPostStrategy_UpdatePost(t *testing.T) {
    // Create a mock controller
    ctrl := gomock.NewController(t)
    defer ctrl.Finish()
